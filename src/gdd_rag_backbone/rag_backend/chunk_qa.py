...
def _get_storage_paths(workspace_id: Optional[str] = None):
    """Get storage paths for a workspace or default to legacy paths."""
    if workspace_id:
        from gdd_rag_backbone.workspace.storage import WorkspaceStorage
        storage = WorkspaceStorage(workspace_id)
        return {
            "status": storage.get_status_file(),
            "chunks": storage.get_chunks_file(),
            "entities": storage.get_entities_file(),
            "vdb_chunks": storage.get_vector_chunks_file(),
            "vdb_entities": storage.get_vector_entities_file(),
            "vdb_relations": storage.get_vector_relations_file(),
            "graph": storage.get_graph_file(),
            "llm_cache": storage.get_llm_cache_file(),
            "parse_cache": storage.get_parse_cache_file(),
        }
    else:
        # Legacy paths
        return {
            "status": STATUS_PATH,
            "chunks": CHUNKS_PATH,
            "entities": DEFAULT_WORKING_DIR / "kv_store_entity_chunks.json",
            "vdb_chunks": VDB_CHUNKS_PATH,
            "vdb_entities": DEFAULT_WORKING_DIR / "vdb_entities.json",
            "vdb_relations": DEFAULT_WORKING_DIR / "vdb_relationships.json",
            "graph": DEFAULT_WORKING_DIR / "graph_chunk_entity_relation.graphml",
            "llm_cache": DEFAULT_WORKING_DIR / "kv_store_llm_response_cache.json",
            "parse_cache": DEFAULT_WORKING_DIR / "kv_store_parse_cache.json",
        }
...
def load_doc_status(workspace_id: Optional[str] = None) -> Dict[str, dict]:
    """Load document status, optionally scoped to a workspace."""
    paths = _get_storage_paths(workspace_id)
    try:
        data = _load_json(paths["status"])
...
def list_indexed_docs(workspace_id: Optional[str] = None) -> List[dict]:
    """List indexed documents, optionally scoped to a workspace."""
...
def get_doc_metadata(doc_id: str, workspace_id: Optional[str] = None) -> Optional[dict]:
    """Get document metadata, optionally scoped to a workspace."""
    return load_doc_status(workspace_id=workspace_id).get(doc_id)


def load_doc_chunks(doc_id: str, workspace_id: Optional[str] = None) -> List[ChunkRecord]:
    """Load document chunks, optionally scoped to a workspace."""
    paths = _get_storage_paths(workspace_id)
    try:
        data = _load_json(paths["chunks"])
    except ChunkStoreError:
        return []
...
def _load_chunk_vectors(doc_ids: Sequence[str], workspace_id: Optional[str] = None) -> Dict[str, List[float]]:
    """Load chunk vectors, optionally scoped to a workspace."""
    paths = _get_storage_paths(workspace_id)
...
def ask_with_chunks(
    ...
    *,
    ...
    metadata = get_doc_metadata(doc_id, workspace_id=workspace_id)
    if metadata is None:
    ...
    question_embedding = _embed_texts(provider, [question])[0]
    vectors = _load_chunk_vectors([doc_id], workspace_id=workspace_id)
...
def ask_across_docs(
    ...
    *,
    ...
    unique_ids = list(dict.fromkeys(doc_ids))
    metadata_map = {doc_id: get_doc_metadata(doc_id, workspace_id=workspace_id) or {} for doc_id in unique_ids}
