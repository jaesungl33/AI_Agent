...
async def _retrieve_context(doc_ids: Sequence[str], query: str, top_k: int, workspace_id: Optional[str] = None) -> Tuple[str, QwenProvider]:
    bundle = _provider_bundle()
    provider = bundle["provider"]

    def _load() -> List[Dict[str, Any]]:
        return get_top_chunks(doc_ids, query, provider=provider, top_k=top_k, workspace_id=workspace_id)
...
async def _extract_list(
    ...
    *,
    ...
    context, _ = await _retrieve_context([doc_id], query, top_k, workspace_id=workspace_id)
...
BEHAVIOR_EXTRACTION_TEMPLATE = """
Convert this GDD requirement into a structured behavior requirement following QA methodology.
Focus on WHAT the system should do (behavior), not HOW it's implemented.

Extract:
- Triggers: What events or conditions cause this behavior to activate?
- Effects: What happens when this behavior executes?
- Entities: What game objects, systems, or actors are involved?
- Conditions: Any preconditions or constraints?
- Priority: high/medium/low

Return ONLY JSON:
{{
  "id": "...",
  "summary": "Brief behavior description",
  "triggers": ["trigger1", "trigger2"],
  "effects": ["effect1", "effect2"],
  "entities": ["Entity1", "Entity2"],
  "conditions": ["condition1"],
  "priority": "high/medium/low",
  "expected_code_anchor": "Optional hint like Class.Method"
}}

GDD Requirement:
{{REQUIREMENT}}

Be precise. Extract actual triggers, effects, and entities from the requirement text.
Do NOT invent details that aren't in the requirement.
"""


async def convert_to_behavior_requirement(
    requirement: GddRequirement,
    *,
    llm_func=None,
) -> BehaviorRequirement:
    """
    Convert a GDD requirement to a structured behavior requirement using QA methodology.
    This extracts triggers, effects, entities, and conditions - the bridge between GDD and code.
    """
    # Build context from requirement
    req_dict = requirement.to_dict()
    req_text = json.dumps(req_dict, indent=2)
    
    prompt = BEHAVIOR_EXTRACTION_TEMPLATE.replace("{{REQUIREMENT}}", req_text)
    system_prompt = (
        "You are a QA engineer converting game design requirements into structured behavior specifications. "
        "Extract triggers, effects, entities, and conditions. Return ONLY JSON."
    )
    
    response_text = await _call_llm(prompt, system_prompt, llm_func=llm_func, temperature=0.1)
    cleaned = _strip_code_fences(response_text)
    
    try:
        payload = json.loads(cleaned)
    except json.JSONDecodeError as exc:
        # Fallback: create basic behavior requirement from existing fields
        return BehaviorRequirement(
            id=requirement.id,
            summary=requirement.summary or requirement.title or requirement.description[:100],
            triggers=requirement.triggers or [],
            effects=requirement.effects or [],
            entities=requirement.entities_involved or requirement.related_objects or [],
            conditions=[],
            priority=requirement.priority,
            expected_code_anchor=requirement.expected_code_anchors[0] if requirement.expected_code_anchors else None,
            source_requirement_id=requirement.id,
        )
    
    return _coerce_dataclass(BehaviorRequirement, {
        **payload,
        "source_requirement_id": requirement.id,
    })


async def extract_behavior_requirements(
    doc_id: str,
    *,
    llm_func=None,
    top_k: int = 10,
) -> List[BehaviorRequirement]:
    """
    Extract all requirements from a GDD and convert them to structured behavior requirements.
    This is Step 1 of the QA-style approach: GDD → Behavior Requirements.
    """
    # First extract regular requirements
    all_data = await extract_all_requirements(doc_id, llm_func=llm_func, top_k=top_k)
    requirements = [
        _coerce_dataclass(GddRequirement, item)
        for item in all_data.get("requirements", [])
        if isinstance(item, dict)
    ]
    
    # Convert each to behavior requirement
    behavior_reqs = []
    for req in requirements:
        try:
            behavior_req = await convert_to_behavior_requirement(req, llm_func=llm_func)
            behavior_reqs.append(behavior_req)
        except Exception as e:
            # Log error but continue with other requirements
            import logging
            logger = logging.getLogger(__name__)
            logger.warning(f"Failed to convert requirement {req.id} to behavior requirement: {e}")
            # Create fallback
            behavior_reqs.append(BehaviorRequirement(
                id=req.id,
                summary=req.summary or req.title or req.description[:100],
                triggers=req.triggers or [],
                effects=req.effects or [],
                entities=req.entities_involved or req.related_objects or [],
                conditions=[],
                priority=req.priority,
                source_requirement_id=req.id,
            ))
    
    return behavior_reqs
